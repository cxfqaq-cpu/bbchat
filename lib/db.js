...
function validateUserId(id) {
  ...
  // 测试账号 1 仅系统预置，禁止用户自行注册占用
  if (normalized === '1') return { ok: false, error: '该账号为系统测试号，请换一个' };
...
function privateConvId(a, b) {
  const norm = (id) => (String(id || '').trim() === '1' ? '1' : normalizeUserId(id));
...
function getOtherUserId(convId, userId) {
  ...
  const me = String(userId || '').trim() === '1' ? '1' : normalizeUserId(userId);
...
async function getUser(id) {
  ...
  const key = String(id).trim() === '1' ? '1' : normalizeUserId(id);
  const { data } = await sb.from('users').select('*').eq('id', key).maybeSingle();
...
function canonUserId(id) {
  const raw = String(id || '').trim();
  return raw === '1' ? '1' : normalizeUserId(raw);
}

async function sendFriendRequest(userId, targetId) {
  const me = canonUserId(userId);
  const target = canonUserId(targetId);
...
async function respondFriendRequest(userId, requestId, action) {
  ...
  const fromId = canonUserId(row.from_id);
  await addFriendship(me, fromId);
  const convId = await ensurePrivateConversation(me, fromId, privateConvId(me, fromId));
...
async function ensureDemoAccount() {
  ...
  const t1 = await getUser('1');
  if (!t1) {
    await sb.from('users').insert({
      id: '1', password: '1', nickname: '测试账号',
      gender: '未填写', email: '未绑定', email_verified: false,
      favorite_games: '未填写', avatar: null
    });
  } else if (t1.password !== '1') {
    await sb.from('users').update({ password: '1' }).eq('id', '1');
  }
}
